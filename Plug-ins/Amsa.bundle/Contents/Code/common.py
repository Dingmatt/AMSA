...
def MapMeta(root):
    ...
    for map in sorted(root.xpath("""./Season/Episode/Mapped/Anidb"""), key=lambda x: int(x.get("series"))):
        if AniDB == None or AniDB.ID != map.get("series"):    
            AniDB = anidb.AniDB(map.get("series"))
            Log("Anidb s title: %s" % (AniDB.Title))
    ...
    TvDB = None    
    for map in sorted(root.xpath("""./Season/Episode/Mapped/Tvdb"""), key=lambda x: int(x.get("series"))):
